        ...
        if data_length == 0:
            ...
            for i in range(0, data_length):
                ...
                mblog_info = (mblogurl, mblog_attitudes,
                              mblog_comment, mblog_repost, mblog_date, mblog_mid, mblog_text, mblog_source, mblog_user_id, mblog_user_name, mblog_user_gender, mblog_user_statuses_count, mblog_user_followers_count, mblog_user_follow_count, mblog_user_verified, mblog_user_verified_type, mblog_text_data)
                mblog_info_list.append(mblog_info)
        print(str(100*page_index/amount)+'%')
    colums = ['mblogurl', ' mblog_attitudes', 'mblog_comment', ' mblog_repost', ' mblog_date', ' mblog_mid', ' mblog_text', ' mblog_source', ' mblog_user_id', ' mblog_user_name',
              ' mblog_user_gender', ' mblog_user_statuses_count', ' mblog_user_followers_count', ' mblog_user_follow_count', ' mblog_user_verified', ' mblog_user_verified_type', 'mblog_text_data']
    mblog_dataframe = pd.DataFrame(columns=colums, data=mblog_info_list)
    try:
        mblog_dataframe.to_csv(key+'_weibo.csv', encoding='utf_8_sig')
    except Exception:
        traceback.print_exc()


if __name__ == '__main__':
    keyword = ['人工智能', '信息化', '网络安全', '数字化转型', '智能化', '虚拟现实', '数字经济', '大数据', '融合发展', '机器人', '信息技术', '知识产权',
               '网络化', '场景创新', '智能制造', '信息基础设施', '云计算', '物联网', '智能化水平', '一体化', '技术创新', '信息系统', '信息共享', '智慧城市']
    province = ['河北', '山西', '辽宁', '吉林', '黑龙江', '江苏', '浙江', '安徽', '福建', '江西', '山东', '河南', '湖北', '湖南', '广东', '海南',
                '四川', '贵州', '云南', '陕西', '甘肃', '青海', '台湾', '内蒙古', '广西', '西藏', '宁夏', '新疆', '北京', '天津', '上海', '重庆', '香港', '澳门']
    keyword_ = keyword[17:]
    province_ = province[0:]
    print(keyword_)
    print(province_)
    year = 2018
    for i in province:
        ...
        for j in keyword_:
            key = i + j
            print("no"+str(c))
            get_response(key=key, page=1, amount=180, year=year)
            c += 1
